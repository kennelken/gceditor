package ru.kennel32.editor.view.components.buttons 
{
	...
	public class BtnDelete extends SimpleButton
	{
		public function BtnDelete(size:int)
		{
		...
		private function drawState(size:int, scale:Number):Shape
		{
			...
			res.graphics.beginFill(Color.RED, 1);
			...
			res.graphics.moveTo(offset + realSize / 4,		offset + realSize / 4);
			res.graphics.lineTo(offset + 3 * realSize / 4,	offset + 3 * realSize / 4);
			
			res.graphics.moveTo(offset + 3 * realSize / 4,		offset + realSize / 4);
			res.graphics.lineTo(offset + realSize / 4,	offset + 3 * realSize / 4);
